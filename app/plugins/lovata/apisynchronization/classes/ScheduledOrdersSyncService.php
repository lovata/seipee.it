...
/**
 * ScheduledOrdersSyncService
 *
 * Syncs scheduled orders from Seipee API (xbtvw_B2B_CFP) into OrderPosition.
 * Each API record represents one order position with all order data included.
 * Orders from CFP are marked with is_scheduled = true.
 */
class ScheduledOrdersSyncService
{
    ...
    /**
     * Sync scheduled orders from Seipee API table xbtvw_B2B_CFP.
    ...
    public function sync(int $rows = 200, bool $useMock = false, string $mockFile = null): array
    {
        ...
        if ($useMock) {
            $this->log('Starting scheduled orders sync from MOCK DATA...');
            $mockData = $this->loadMockData($mockFile);
            $dataSource = [$mockData]; // Wrap in array to simulate pagination
        } else {
            $this->log('Starting scheduled orders sync from xbtvw_B2B_CFP...');
            $dataSource = $this->api->paginate('xbtvw_B2B_CFP', $rows);
        }
        ...
        try {
                ...
                try {
                    ...
                    foreach ($list as $row) {
                        try {
                            ...
                            Log::error('ScheduledOrdersSyncService error: '.$e->getMessage(), [
                                'row' => $row ?? null,
            ...
            Log::error('ScheduledOrdersSyncService critical error: '.$e->getMessage(), [
                'trace' => $e->getTraceAsString(),
            ]);
        }
    ...
    {
        if ($mockFile === null) {
            $mockFile = plugins_path('lovata/apisynchronization/mock_scheduled_orders.json');
    ...
    /**
     * Sync undelivered scheduled orders and mark as delivered those that are missing from API.
     *
     * Logic:
     * 1. Fetch all undelivered scheduled orders from API (DocEvaso = false)
     * 2. Track processed seipee_order_id
     * 3. Find orders in our DB that are marked as undelivered and scheduled (is_delivered = false, is_scheduled = true)
    ...
    public function syncUndelivered(int $rows = 200, bool $useMock = false, string $mockFile = null): array
    {
        ...
        if ($useMock) {
            $this->log('Starting undelivered scheduled orders sync from MOCK DATA...');
            $mockData = $this->loadMockData($mockFile);
            ...
            $this->log('Starting undelivered scheduled orders sync (DocEvaso = false)...');
            // Filter: both DVI and OCI, but only undelivered (DocEvaso = false)
            $where = "DocEvaso = 0";
            $dataSource = $this->api->paginate('xbtvw_B2B_CFP', $rows, $where);
        }
        ...
        try {
                ...
                try {
                    ...
                    foreach ($list as $row) {
                        try {
                            ...
                            $errors++;
                            $this->log('Error processing row: '.$e->getMessage(), 'error');
                            Log::error('ScheduledOrdersSyncService (undelivered) error: '.$e->getMessage(), [
                                'row' => $row ?? null,
                                'trace' => $e->getTraceAsString(),
                            ]);
                        }
            ...
            Log::error('ScheduledOrdersSyncService critical error (undelivered): '.$e->getMessage(), [
                'trace' => $e->getTraceAsString(),
            ]);
        }

        $this->log('Undelivered scheduled orders sync completed. Checking for delivered orders...');

        // Find orders in DB marked as undelivered, scheduled but not in API response
        ...
        foreach ($undeliveredInDb as $seipeeOrderId) {
            ...
            if (!isset($processedSeipeeIds[$seipeeOrderId])) {
                $this->log('Scheduled order '.$seipeeOrderId.' is now delivered. Syncing...');
                ...
                try {
                    ...
                    $errors++;
                    $this->log('Error syncing delivered scheduled order '.$seipeeOrderId.': '.$e->getMessage(), 'error');
                    Log::error('ScheduledOrdersSyncService (delivered order sync) error: '.$e->getMessage(), [
        ...
        $this->log("Sync completed! Found {$deliveredCount} scheduled orders that became delivered.");
        return [
            'createdOrders' => $createdOrders,
            'updatedOrders' => $updatedOrders,
            'createdPositions' => $createdPositions,
    ...
    /**
     * Sync a specific scheduled order by its Seipee ID.
    ...
    {
        try {
            $this->log("Fetching scheduled order {$seipeeOrderId} from API...");

            // Fetch order data from API
            $where = "ID_DOTes = {$seipeeOrderId}";
            $data = $this->api->fetch('xbtvw_B2B_CFP', 1, 100, $where);
            $list = Arr::get($data, 'result', []);

            if (empty($list)) {
                $this->log("Scheduled order {$seipeeOrderId} not found in API", 'warning');
            ...
            if ($order) {
                $this->log("Scheduled order {$seipeeOrderId} synced successfully ({$createdPositions} created, {$updatedPositions} updated positions)");
            ...
            $this->log('Error syncing scheduled order '.$seipeeOrderId.': '.$e->getMessage(), 'error');
     ...
     * Sets is_scheduled = true for orders from CFP.
     *
    ...
    protected function findOrCreateOrder(array $row, int $idDOTes): array
    {
        ...
        if (!$order && $numeroDoc) {
            ...
            if ($order && !$order->seipee_order_id) {
                ...
                $order->save();
                $this->log('Updated seipee_order_id for existing scheduled order: '.$numeroDoc);
            }
        }
        ...
        if (!$order) {
            ...
            // Mark as scheduled order from CFP
            ...
            $this->log('Created scheduled order: '.$numeroDoc.' (Seipee ID: '.$idDOTes.')');
        } else {
            ...
            if ($updated) {
                $order->save();
                $this->log('Updated scheduled order: '.$numeroDoc);
            }
        }
    ...
    public function findOrCreateOrderPosition(Order $order, array $row, int $idDORig): array
    {
        ...
        if ($prezzoUnitario !== null) {
            ...
            $position->price = 0;
            $this->log("Price is null for position {$cdAR} in scheduled order {$order->order_number}, setting to 0", 'warning');
        }
        ...
        $position->save();
        if ($created) {
            $this->log('Created position: '.$cdAR.' for scheduled order '.$order->order_number);
        } elseif ($updated) {
            $this->log('Updated position: '.$cdAR.' for scheduled order '.$order->order_number);
        }
    ...
    /**
     * Update order totals from positions
     *
     * @param Order $order
     * @return void
     */
    ...
    {
        // This method is called from SyncUndeliveredScheduledOrders
