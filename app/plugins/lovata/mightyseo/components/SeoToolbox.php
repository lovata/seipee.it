...
class SeoToolbox extends ComponentBase
{
    ...
    {
        $sResult = $this->get('seo_title');
        if (empty($sResult)) {
            $obActiveSite = Site::getActiveSite();
            $sResult = !empty($obActiveSite) ? Arr::get($this->page->page->localeMeta_title, $obActiveSite->code, $this->page->page->meta_title) : $this->page->page->meta_title;
        }

        $sResult = Settings::get('seo_title_prefix').' '.$sResult.' '.Settings::get('seo_title_suffix');
    ...
    {
        $sResult = $this->get('seo_description');
        if (empty($sResult)) {
            $obActiveSite = Site::getActiveSite();
            $sResult = !empty($obActiveSite) ? Arr::get($this->page->page->localeMeta_description, $obActiveSite->code, $this->page->page->meta_description) : $this->page->page->meta_description;
        }
    ...
    public function getPageTitle($obModel = null, $arTemplateParams = null)
    {
        ...
        if (empty($sResult)) {
            $obActiveSite = Site::getActiveSite();
            $sResult = !empty($obActiveSite) ? Arr::get($this->page->page->localeTitle, $obActiveSite->code, $this->page->page->title) : $this->page->page->title;
        }
    ...
    {
        $sResult = $this->get('og_title');
        if (empty($sResult)) {
            $sResult = $this->getMetaTitle();
        }
    ...
    {
        $sResult = $this->get('og_description');
        if (empty($sResult)) {
            $sResult = $this->getMetaDescription();
        }
    ...
    protected function getModelField($sKey)
    {
        ...
        if (!$this->isNeedProcess($sKey)) {
            $sFieldValue = is_string($sFieldValue) ? trim($sFieldValue) : $sFieldValue;
    ...
    {
        return !in_array($sKey, ['robots_follow', 'robots_index', 'robots_index_addition', 'canonical_url', 'og_image']);
    }
}
