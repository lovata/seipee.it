...
class BoxesEditor extends FormWidgetBase
{
    ...
    /**
     * Marks if the "single" mode is allowed.
     */
    protected bool $allowSingleMode = true;
    ...
    public function init()
    {
        ...
        if ($this->isSingleMode() && !$this->resolveRelatedContentModel() && !$this->isRenderedInRelationWidget()) {
    ...
    /**
     * Publishes many pages at once.
     */
    public function onBatchPublish()
    {
        $ids = post('ids', []);
    ...
    public function onCopyPageToSite()
    {
        $page = Page::withoutGlobalScope(MultisiteScope::class)->findOrFail(post('page_id'));
        $site = Site::getSiteFromId(post('site_id'));

        if (!$site) {
            Flash::error('Unknown site');

            return null;
        }

        Site::setActiveSiteId($site->id);

        $url = \Backend\Facades\Backend::url(sprintf('offline/boxes/editorcontroller?_site_id=%d&boxes_page=', $site->id));

        // The page already exists on the target site.
        $otherSiteModel = $page->newOtherSiteQuery()->where($page->getSiteIdColumn(), $site->id)->where('theme', $site->theme ?? $page->theme)->first();
        ...
        // Duplicate the page.
        return Site::withGlobalContext(function () use ($page, $site, $url) {
            $page->useNestedTreeStructure = false;
            $page->{$page->getSiteIdColumn()} = $site->id;

            $parentModel = Page::withoutGlobalScope(MultisiteScope::class)->find($page->parent_id);

            $page->parent_id = null;

            $copy = $page->duplicate(true, true);

            $copy->site_root_id = $page->site_root_id ?: $page->id;
            $copy->origin_page_id = $page->origin_page_id ?: $page->id;
            $copy->slug = $page->slug ?: $copy->slug;
            $copy->parent_id = null;

            if ($site->theme) {
                $copy->theme = $site->theme;
            }

            if ($parentModel && $newParentModel = $parentModel->findForSite($site->id)) {
                $copy->parent_id = $newParentModel->id;
            }
            ...
            // If the original page is the primary site's root page, set the page to `this_page_is_root`.
            if ($page->site_id === Site::getPrimarySite()->id) {
                $page->site_root_id = $page->id;
                $page->save();
            }

            return ['redirect' => $url . $copy->id];
        });
    }
    
    public function onShowRevisionDetail()
    {
        $revision = Page::withoutGlobalScopes()->findOrFail(post('id'));

        $previewUrl = url()->to(Site::getSiteFromContext()?->base_url) . '/' . trim($revision->url ?: '_revision', '/');

        $previewUrl .= '?' . http_build_query([
            Controller::DRAFT_ID_PARAM => $revision->id,
        ]);

        return $this->makePartial('$/offline/boxes/controllers/editorcontroller/_revision_popup', [
            'revision' => $revision,
            'previewUrl' => $previewUrl,
        ]);
    }
    
    public function onRestoreRevision()
    {
        $revision = Page::withoutGlobalScopes()->findOrFail(post('id'));

        $restored = $revision->restoreAsDraft();

        return redirect(\Backend\Facades\Backend::url('offline/boxes/editorcontroller?boxes_page=' . $restored->id));
    }
    ...
    /**
     * Fetch the data for the publishing popup.
     */
    protected function getPublishingInfo(Page $page, ?Page $current, array $reload = [])
    {
        $build = function (?Page $page) {
            if (!$page) {
                return null;
            }
            ...
            return [
                'id' => $page->id,
                'date_string' => $page->updated_at->diffForHumans(['parts' => 1]),
                'date_time' => $page->updated_at->toDateTimeString(),
                'version' => sprintf('%03s', $page->version),
                'author' => $author,
            ];
        };
