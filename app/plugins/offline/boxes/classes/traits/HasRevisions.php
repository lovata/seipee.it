 ...
 * @method static Builder currentDrafts(?int $site = null)
 * @method static Builder current(?int $site = null)
 * @method static Builder currentPublished(?int $site = null)
 */
trait HasRevisions
{
    ...
    public function publishDraft(): Page
    {
        ...
        $published->site_root_id = $this->site_root_id;
     ...
     * @param mixed $query
     */
    public function scopeCurrent($query, ?int $siteId = null)
    {
        $state = BackendAuth::getUser() ? PublishedState::DRAFT : PublishedState::PUBLISHED;

        return $this->scopeCurrentByPublishingState($query, $state, $siteId);
    }

    public function scopeCurrentPublished($query, ?int $siteId = null)
    {
        return $this->scopeCurrentByPublishingState($query, PublishedState::PUBLISHED, $siteId);
    }

    public function scopeCurrentDrafts($query, ?int $siteId = null)
    {
        return $this->scopeCurrentByPublishingState($query, PublishedState::DRAFT, $siteId);
    }

    public function scopeCurrentByPublishingState($query, string $state, ?int $siteId = null)
    {
        // If the revisions feature is disabled, always return the draft.
        if (!Features::instance()?->isEnabled('revisions')) {
            $state = PublishedState::DRAFT;
        }

        $siteId ??= Site::getSiteIdFromContext();

        return $query
            ->when($siteId && $siteId !== -1, fn ($q) => $q->where('site_id', $siteId))
